        ...
        Task.WhenAll(
			...
			.ContinueInMainThreadWith((t8c)=>{
                if(t8c.IsFaulted)
				{
					...
					while (e0 is Firecoals.AggregateException) e0 = ((Firecoals.AggregateException)e0).InnerExceptions[0];
					Exception e1 = t8c.Exception.InnerExceptions[1];
					while (e1 is Firecoals.AggregateException) e1 = ((Firecoals.AggregateException)e1).InnerExceptions[0];
