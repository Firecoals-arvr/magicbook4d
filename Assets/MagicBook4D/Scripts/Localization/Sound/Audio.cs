    ...
    public class Audio
    {
        ...
        public Transform SourceTransform
        {
            ...
            set
            {
                if (value == null)
                {
                    sourceTransform = FirecoalsSoundManager.Gameobject.transform;
                }
                else
                {
        ...
        public Audio(AudioType audioType, AudioClip clip, bool loop, bool persist, float volume, float fadeInValue, float fadeOutValue, Transform sourceTransform)
        {
            ...
            Pitch = 1;
            StereoPan = 0;
            if (sourceTransform != null && sourceTransform != FirecoalsSoundManager.Gameobject.transform)
            {
                SpatialBlend = 1;
            }
            ReverbZoneMix = 1;
        ...
        public void Play(float volume)
        {
            ...
            if (Pooled)
            {
                ...
                bool restoredFromPool = FirecoalsSoundManager.RestoreAudioFromPool(Type, AudioID);
                if (!restoredFromPool)
                {
                    return;
                }
        ...
        public void Update()
        {
            ...
            switch (Type)
            {
                case AudioType.Music:
                    {
                        AudioSource.volume = Volume * FirecoalsSoundManager.GlobalMusicVolume * FirecoalsSoundManager.GlobalVolume;
                        break;
                    }
                case AudioType.Sound:
                    {
                        AudioSource.volume = Volume * FirecoalsSoundManager.GlobalSoundsVolume * FirecoalsSoundManager.GlobalVolume;
                        break;
                    }
                case AudioType.UISound:
                    {
                        AudioSource.volume = Volume * FirecoalsSoundManager.GlobalUISoundsVolume * FirecoalsSoundManager.GlobalVolume;
                        break;
                    }
            }
